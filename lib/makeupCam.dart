    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          return TextButton(
              onPressed: () {
                setState(() {
                  _currentSelectedPattern = list[index]["guid"];
                });
                makeupCamViewChannel.apply(const VtoSetting().create(
                    _currentSelectedProduct!,
                    _currentSelectedSku!,
              ...
              style: TextButton.styleFrom(minimumSize: const Size(50, 80)),
              child: FutureBuilder(
                  future: patternCell(context, list[index],
                      _currentSelectedPattern == list[index]["guid"]),
                  builder: (BuildContext context,
                      AsyncSnapshot<Widget> patternList) {
                    return SizedBox(
                        height: 45, width: 45, child: patternList.data);
                  }));
        });
  }

  Future<Widget> patternCell(
      BuildContext context, Map pattern, bool isSelected) async {
    if (pattern["thumbnail"] == null || pattern["thumbnail"] == "") {
      return Container();
    }
    return Container(
      height: 20,
